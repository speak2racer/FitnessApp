...
if not daten.empty:
    ...
    if not vorwoche.empty:
        ...
        symbol = "📉" if gewicht_trend < -0.1 else "📈" if gewicht_trend > 0.1 else "➡️"
    ...
    st.warning("⚠️ Noch keine Gewichtsdaten vorhanden.")

# Erinnerungen kompakt oben
hinweise = []
if daten.empty or daten[daten["Datum"].dt.normalize() == heute].empty:
    hinweise.append("⚖️ Kein Gewicht heute")
if nutrition_heute.empty:
    hinweise.append("🍽️ Keine Ernährung heute")
if hinweise:
    st.warning("  |  ".join(hinweise))

# ── TABS ──────────────────────────────────────────────────────────────────────
tab_heute, tab_woche, tab_koerper = st.tabs(["📅 Heute", "📊 Woche", "💪 Körper"])
